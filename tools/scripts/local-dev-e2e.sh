...
Options:
  --artifacts-port <port>   Static artifact server port (default: 18081)
...
4. serves the artifacts directory locally
5. runs fontpub list/show/install/status/verify against the local artifacts
EOF
}
...
python3 -m http.server "${ARTIFACTS_PORT}" --bind 127.0.0.1 --directory "${ARTIFACTS_DIR}" >"${LOG_DIR}/artifact-server.log" 2>&1 &
SERVER_PID="$!"
for _ in $(seq 1 50); do
  if curl -fsS "http://127.0.0.1:${ARTIFACTS_PORT}/v1/index.json" >/dev/null 2>&1; then
    break
  fi
  sleep 0.1
done
curl -fsS "http://127.0.0.1:${ARTIFACTS_PORT}/v1/index.json" >/dev/null

FONTPUB_ENV=(
  "GOCACHE=${GOCACHE_DIR}"
  "FONTPUB_BASE_URL=http://127.0.0.1:${ARTIFACTS_PORT}"
...
echo "== fontpub list --json =="
(cd "${GO_DIR}" && env "${FONTPUB_ENV[@]}" go run ./cmd/fontpub list --json)
echo
...
echo "== fontpub status ${LOWER_PACKAGE_ID} --json =="
(cd "${GO_DIR}" && env "${FONTPUB_ENV[@]}" go run ./cmd/fontpub status "${LOWER_PACKAGE_ID}" --json)
echo
